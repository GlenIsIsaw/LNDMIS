                                                    ...
                                                    <div class="btn-group dropstart">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            ...
                                                            @if ($idp->submit_status == 'Not Submitted' || $idp->submit_status == 'Rejected')
                                                                <li><button type="button" data-bs-toggle="modal" data-bs-target="#submitIdpModal" wire:click="getId({{$idp->idp_id}})" class="btn-success text-white rounded-3 shadow-sm fw-bold px-3 py-2 text-uppercase dropdown-item" style="background-image: linear-gradient(
                                                                    to top, #000000,
                                                                    #0f9b0f); font-size:15px; padding:11px 11px 11px 11px;"><i class="fas fa-paper-plane me-1"></i>Submit</button></li>
                                                            @endif
                                                            <li><button type="button" wire:click="show({{$idp->idp_id}})" class="btn-success rounded-3 shadow-sm fw-bold text-white text-uppercase px-3 py-2 dropdown-item" style="background-image: linear-gradient(
                                                                to bottom, #52c234,
                                                                        #061700);"><i class="fas fa-eye me-1"></i>View</button></li>
                                                            <li><button type="button" data-bs-toggle="modal" data-bs-target="#printIdpModal" wire:click="signature({{$idp->idp_id}})" class="btn-info text-white btn-sm rounded-3 shadow-sm fw-bold text-uppercase dropdown-item" style="background-image: linear-gradient(
                                                                to bottom, #43C6AC,
                                                                    #191654); font-size:13px; padding:10px 10px 10px 10px;"><i class="fas fa-download me-1"></i>Download</button></li>
                                                            @if ($idp->comment)
                                                            <li><button type="button" data-bs-toggle="modal" data-bs-target="#showCommentModal" wire:click="showComment({{$idp->idp_id}})" class="btn-info text-white rounded-3 shadow text-uppercase fw-bold px-3 py-2 dropdown-item" style="background-image: linear-gradient(
                                                                    to bottom, #43C6AC,
                                                                    #191654);font-size:13px; padding:11px 11px 11px 11px;"><i class="fas fa-comments me-1"></i>Comment</button></li>
                                                            @endif
                                                            ...
                                                            @if ($idp->submit_status != 'Approved') 
                                                                @if ($idp->submit_status == 'Pending')
                                                                    ...
                                                                    <li><button type="button" data-bs-toggle="modal" data-bs-target="#approveIdpModal" wire:click="getId({{$idp->idp_id}})" class="btn-success btn-sm text-white rounded-3 shadow fw-bold px-2 py-2 text-uppercase dropdown-item" style="background-image: linear-gradient(
                                                                            to top, #000000,
                                                                            #0f9b0f);"><i class="fas fa-thumbs-up me-1"></i>Approve</button></li>

                                                                        <li><button type="button" data-bs-toggle="modal" data-bs-target="#rejectIdpModal" wire:click="getId({{$idp->idp_id}})" class="btn-danger text-white rounded-3 shadow fw-bold px-2 py-2 text-uppercase dropdown-item" style="background-image: linear-gradient(
                                                                            to bottom, #870000,
                                                                            #190A05);"><i class="fas fa-thumbs-down me-1"></i>Reject</button></li>
                                                                    @endif    
                                                                @endif

                                                                    
                                                                @if($idp->submit_status != 'Pending')
                                                                    <li><button type="button" wire:click="edit({{$idp->idp_id}})" class="btn-primary text-white rounded-3 text-uppercase shadow-sm fw-bold px-3 py-2 dropdown-item" style="background-image: linear-gradient(
                                                                            to bottom, #000046, 
                                                                            #1CB5E0);"><i class="fas fa-edit me-1"></i>Edit</button></li>
                                                                    <li><button type="button" data-bs-toggle="modal" data-bs-target="#deleteIdpModal" wire:click="getId({{$idp->idp_id}})" class="btn-danger text-white text-uppercase rounded-3 shadow-sm px-3 fw-bold py-2 dropdown-item" style="background-image: linear-gradient(
                                                                            to bottom, #870000,
                                                                    ...
                                                                    @if ($idp->submit_status == 'Pending')
                                                                        @if (auth()->user()->role_as == 0)
                                                                            <li><button type="button" data-bs-toggle="modal" data-bs-target="#removeSubmissionIdpModal" wire:click="getId({{$idp->idp_id}})" class="btn-danger text-white rounded-3 shadow-sm px-3 fw-bold py-2 px-3 text-uppercase dropdown-item" style="background-image: linear-gradient(
                                                                                to bottom, #870000,
                                                                                #190A05);"><i class="fas fa-trash"></i><br>Remove Submission</button></li>
                                                                        @endif    
                                                                    @endif
                                                            @endif
                                                        </ul>
